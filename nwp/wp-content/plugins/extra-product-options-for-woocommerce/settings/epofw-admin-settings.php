...
$get_post_id = '';
if ( 'edit' === $get_action ) {
	$get_post_id = filter_input( INPUT_GET, 'post' );
...
?>
	<h1 class="wp-heading-inline">
		<?php echo esc_html__( 'Add Product Addon', 'extra-product-options-for-woocommerce' ); ?>
	</h1>
									...
									<div class="forminp">
										<div class="forminp1">
											<select name="epofw_data[epofw_addon_position]" id="epofw_addon_position">
												<option value="before_add_to_cart" <?php selected( 'before_add_to_cart', $check_epofw_addon_position ); ?>>
													<?php esc_html_e( 'Before Add to Cart', 'extra-product-options-for-woocommerce' ); ?>
												</option>
												<option value="after_add_to_cart" <?php selected( 'after_add_to_cart', $check_epofw_addon_position ); ?>>
													<?php esc_html_e( 'After Add to Cart', 'extra-product-options-for-woocommerce' ); ?>
												</option>
											</select>
										</div>
									</div>
								...
								<div class="addon_sub_field_data_div">
									<div class="addon_fields_sub_div">
										...
										<div class="addon_fields_accordian_data">
											<?php
											if ( ! empty( $check_general_data ) && ! empty( $get_data ) && isset( $get_data['general'] ) ) {
												$i = 0;
