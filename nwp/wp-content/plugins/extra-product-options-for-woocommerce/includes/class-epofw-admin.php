...
if ( ! class_exists( 'EPOFW_Admin' ) ) {
	...
	class EPOFW_Admin {
		 ...
		 *
		 * @since    1.0.0
		 * @var      string $epofw_instance Object of the class.
		 */
		protected static $epofw_instance = null;
		/**
		...
		public static function instance() {
			if ( is_null( self::$epofw_instance ) ) {
				self::$epofw_instance = new self();
			}
			return self::$epofw_instance;
		}
		/**
		...
		public function epofw_init() {
			...
			add_action(
				'wp_ajax_nopriv_epofw_change_field_basedon_type',
				array(
					$this,
		...
		public function epofw_get_data_based_on_cd() {
			$request_value     = filter_input( INPUT_POST, 'value' );
			$post_value        = isset( $request_value ) ? sanitize_text_field( epofw_get_cyric_string_to_latin( $request_value ) ) : '';
			$current_condition = filter_input( INPUT_POST, 'current_condition' );
		...
		public function epofw_change_field_basedon_type() {
			...
			$current_val     = filter_input( INPUT_POST, 'current_val' );
			$get_post_id     = filter_input( INPUT_POST, 'post' );
		...
		public function epofw_disbale_field_options() {
			...
			$field_ids   = filter_input( INPUT_POST, 'field_ids', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
			$get_data    = epofw_get_data_from_db( $get_post_id );
			if ( ! empty( $get_data ) && ! empty( $field_ids ) ) {
