...
if ( ! class_exists( 'EPOFW_Front' ) ) {
	...
	class EPOFW_Front {
		 ...
		 *
		 * @since    1.0.0
		 * @var      string $epofw_front_instance Object of the class.
		 */
		protected static $epofw_front_instance = null;
		/**
		...
		public static function instance() {
			if ( is_null( self::$epofw_front_instance ) ) {
				self::$epofw_front_instance = new self();
			}
			return self::$epofw_front_instance;
		}
		/**
		...
		public function cop_front_enqueue_scripts_fn() {
			...
			wp_register_style(
				'jquery-ui-css',
				EPOFW_PLUGIN_URL . 'assets/css/jquery-ui.min.css',
				'',
				'all'
			);
			...
			if ( $prd_wp_query->have_posts() ) {
				foreach ( $prd_wp_query->posts as $f_id ) {
					...
					if ( ! empty( $get_data ) ) {
						...
						if ( isset( $aadditional_rule_data['value'] ) ) {
							$aadditional_rule_data['value'] = array_map( 'intval', $aadditional_rule_data['value'] );
						}
						if (
							$aadditional_rule_data &&
							isset( $aadditional_rule_data['condition'] ) &&
							isset( $aadditional_rule_data['operator'] ) &&
							isset( $aadditional_rule_data['value'] )
						) {
							if ( 'product' === $aadditional_rule_data['condition'] ) {
								if ( 'is_equal_to' === $aadditional_rule_data['operator'] ) {
									if ( in_array( $current_prd_id, $aadditional_rule_data['value'], true ) ) {
										$matched_id_arr[] = $f_id;
									}
								} else {
									if ( ! in_array( $current_prd_id, $aadditional_rule_data['value'], true ) ) {
								...
								if ( 'is_equal_to' === $aadditional_rule_data['operator'] ) {
									if ( ! empty( $cart_product_category ) ) {
										foreach ( $cart_product_category as $cat_id ) {
											if ( in_array( $cat_id, $aadditional_rule_data['value'], true ) ) {
												$matched_id_arr[] = $f_id;
											}
										}
									}
		 ...
		 *
		 * @param array  $fields_data_arr Array of the fields.
		 * @param array  $product_data    Array of the product.
		 *
		 * @return HTML $prd_html Html of the addon field.
		 *
		 * @since 1.0.0
		 */
		public function epofw_get_html_field( $fields_data_arr, $product_data ) {
			$prd_html  = '';
				...
				foreach ( $fields_data_arr as $fields_data ) {
					...
					if ( 'on' === $check_field_status ) {
						...
						if ( ! in_array( $check_field_inp_type, $get_exftpoc, true ) ) {
							...
							$prd_html .= cp_render_fields_front( $check_field_input, $check_field_inp_type, $product_data );
							$prd_html .= '</td>';
						}
						$prd_html .= '</tr>';
					}
		 ...
		 * @since 1.0.0
		 */
		public function epofw_get_addon_detail_html( $product_data ) {
			$product_name  = '';
			$product_price = '';
			...
			if ( ! empty( $main_fields_data_arr ) ) {
				foreach ( $main_fields_data_arr as $fields_data_arr ) {
					...
					if ( 'before_add_to_cart' === $addon_position ) {
						if ( ! empty( $fields_data_arr ) ) {
							echo $this->epofw_get_html_field( $fields_data_arr, $product_data ); // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped
						}
					}
				}
			}
		...
		public function epofw_after_add_to_cart_button() {
			...
			if ( ! empty( $main_fields_data_arr ) ) {
				...
				echo $this->epofw_get_addon_detail_html( $product_data );
			}
		}
		/**
		 * Exclude field types which not include in meta section.
		 *
		 * @return array $exclude_ftocp array of excluding data.
		 *
		 * @since    1.0.0
		 */
		 ...
		 *
		 * @param array $passed     Store in array which is passed.
		 * @param int   $product_id Id of the product id.
		 *
				...
				foreach ( $fields_data_arr as $fields_data ) {
					...
					if ( $check_field_input ) {
						...
						if ( isset( $_POST[ $check_field_name ] ) ) {
							$post_field_name = trim( sanitize_text_field( wp_unslash( $_POST[ $check_field_name ] ) ) );
							if ( $check_field_restriction ) {
								if ( 'only_number' === $check_field_restriction ) {
									if ( 1 !== preg_match( '/^[0-9]*$/', $post_field_name ) ) {
								...
								if ( 'all_number' === $check_field_restriction ) {
									if ( 1 !== preg_match( '/^[0-9]+(\\.[0-9]+)]*$/', $post_field_name ) ) {
								...
								if ( 'only_text' === $check_field_restriction ) {
									if ( 1 !== preg_match( '/^[a-zA-Z ]*$/i', $post_field_name ) ) {
								...
								if ( 'text_number' === $check_field_restriction ) {
									if ( 1 !== preg_match( '/^[a-zA-Z0-9 ]*$/i', $post_field_name ) ) {
								...
								if ( 'email' === $check_field_restriction ) {
									if ( ! filter_var( $post_field_name, FILTER_VALIDATE_EMAIL ) ) {
									...
									if ( isset( $_POST[ $check_field_name ] ) ) {
										...
										if ( ! in_array( $check_field_inp_type, $get_exftpoc, true ) ) {
											...
											if ( is_array( $_POST[ $check_field_name ] ) ) {
												...
												$post_check_field_name           = trim( sanitize_text_field( wp_unslash( $_POST[ $check_field_name ] ) ) );
				...
				foreach ( $cart_obj->get_cart() as $value ) {
					if ( isset( $value['epofw_data'] ) ) {
						if ( isset( $value['product_id'] ) ) {
							...
							if ( $product ) {
								$product_price = epofw_display_product_price( $product, '' );
						...
						if ( ! empty( $value ) && isset( $value['epofw_data'] ) ) {
				...
				if ( $meta_data ) {
					...
					foreach ( $meta_data as $meta_data_value ) {
						if ( array_key_exists( $meta_data_value->key, $get_field_name_arr ) ) {
							$get_display_key              = $this->epofw_get_addon_title( $get_field_name_arr, $meta_data_value->key, $meta_data_value->value );
							$data_arr                     = array();
		 ...
		 *
		 * @param array $main_fields_data_arr Array of fields data which fetched from DB.
		 *
				...
				if ( $formatted_meta ) {
					foreach ( $formatted_meta as $key => $meta ) {
						if ( array_key_exists( $meta->key, $get_field_name_arr ) ) {
							$get_display_key        = $this->epofw_get_addon_title( $get_field_name_arr, $meta->key, $meta->value );
		 ...
		 * @since    1.0.0
		 */
		public function epofw_get_addon_title( $get_field_name_arr, $meta_key, $meta_value ) {
			if ( ! empty( $meta_key ) ) {
