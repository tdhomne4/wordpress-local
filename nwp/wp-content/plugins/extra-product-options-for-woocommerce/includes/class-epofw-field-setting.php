...
if ( ! class_exists( 'EPOFW_Field_Setting' ) ) {
	...
	class EPOFW_Field_Setting {
		 ...
		 *
		 * @since    1.0.0
		 * @var      string $epofw_field_setting_instance Object of the class.
		 */
		protected static $epofw_field_setting_instance = null;
		/**
		 * Post type.
		 *
		...
		public static function instance() {
			if ( is_null( self::$epofw_field_setting_instance ) ) {
				self::$epofw_field_setting_instance = new self();
			}
			return self::$epofw_field_setting_instance;
		}
		/**
		 * Constructor.
		 *
		...
		public static function epofw_output() {
			...
			$epofw_nonce     = filter_input( INPUT_GET, 'epofw_nonce' );
			$get_epofw_add   = filter_input( INPUT_GET, '_wpnonce' );
		...
		private static function epofw_save_method( $method_id = 0 ) {
			$action                        = filter_input( INPUT_GET, 'action' );
			$epofw_save                    = filter_input( INPUT_POST, 'epofw_save' );
			$woocommerce_save_method_nonce = filter_input( INPUT_POST, 'woocommerce_save_method_nonce' );
			if ( ( isset( $action ) && ! empty( $action ) ) ) {
				if ( isset( $epofw_save ) ) {
					...
					$epofw_data            = filter_input( INPUT_POST, 'epofw_data', FILTER_DEFAULT, FILTER_REQUIRE_ARRAY );
		...
		public static function epofw_list_methods_screen() {
			...
			$request_s = filter_input( INPUT_POST, 's' );
