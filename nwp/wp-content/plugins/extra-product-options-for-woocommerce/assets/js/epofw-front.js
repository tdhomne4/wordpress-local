( function ($) {
	...
	function createTr (fieldID, trFieldName, fieldPrice) {
		...
		var addonPrice = addonPriceChangeToDecimalForCalc( fieldPrice );
		...
		firstTrSecondTd.setAttribute( 'data-addon-og-price', fieldPrice );
	...
	function addonPriceChangeToDecimalForCalc( fieldPrice ) {
	...
	function addTrInAddonDetails (fieldValue, fieldID, trFieldName, fieldPrice) {
		...
		if ('' !== fieldValue) {
			var checkTr = mainTableID.find('tr#' + fieldID).length;
			if (0 === checkTr) {
				firstTr = createTr(fieldID, trFieldName, fieldPrice);
			} else {
				mainTableID.find('tr#' + fieldID).find('td:first').html(trFieldName);
				fieldPrice = addonPriceChangeToDecimalForCalc( fieldPrice );
				if (undefined !== fieldPrice) {
					fieldPrice = number_format(fieldPrice, decimals, decimalSeparator, thousandSeparator);
				}
				var fieldPriceWithCurrency;
				...
				mainTableID.find('tr#' + fieldID).find('td:last').html(fieldPriceWithCurrency );
				mainTableID.find('tr#' + fieldID).find('td:last').attr('data-addon-price',  fieldPrice );
			}
			$('#addon_total #addon_subtotal').before(firstTr);
		} else {
			mainTableID.find('tr#' + fieldID).remove();
		}
	}
	...
	var AWS_Front = {
		init: function () {
			...
			$('.epofw_dft_colorpicker').click(function () {
				$('.epofw_dft_colorpicker').iris('hide');
				$(this).iris('show');
				return false;
			});
			...
			if ( $( 'input[type="radio"]:checked', '.epofw_fields_table' ).length ) {
				var radioObject = $( 'input[type="radio"]:checked');
				var fieldValue = radioObject.val();
				var fieldID = radioObject.attr( 'id' );
				var fieldName = radioObject.attr( 'data-label-name' );
				var trFieldName = fieldName + ' - ' + fieldValue;
				var fieldPrice = radioObject.attr( 'addon_price' );
				console.log( ' fieldValue ' + fieldValue );
				console.log( ' fieldID ' + fieldID );
				console.log( ' trFieldName ' + trFieldName );
				console.log( ' fieldPrice ' + fieldPrice );
		...
		onKeyUpChangeAddon: function () {
			...
			if ('checkbox' === fieldType) {
				...
				fieldValue = $(this).val();
				fieldID = $(this).attr('id');
				fieldInpName = $(this).attr('name');
				fieldValue = $('input[name=' + fieldInpName + ']:checked').val();
				trFieldName = fieldName + ' - ' + fieldValue;
