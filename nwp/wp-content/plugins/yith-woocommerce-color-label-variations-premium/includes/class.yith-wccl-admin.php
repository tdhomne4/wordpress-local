	...
	class YITH_WCCL_Admin {
		...
		public function add_attribute_field( $taxonomy ) {
			...
			$values = array(
				'value'   => array(
					'value' => false,
					'label' => isset( $this->custom_types[ $attribute->attribute_type ] ) ? $this->custom_types[ $attribute->attribute_type ] : __( 'Value', 'yith-woocommerce-color-label-variations' ),
					'desc'  => '',
				),
				'tooltip' => array(
					'value' => false,
		...
		public function print_attribute_type( $type, $args, $table = false ) {
			...
			foreach ( $args as $key => $arg ) :
				...
				<input type="text" class="ywccl" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $id ); ?>"
						value="<?php echo esc_attr( $value ); ?>" <?php echo $data; //phpcs:ignore
						?>
				/>
				...
				<input type="text" class="ywccl hidden_empty" name="<?php echo esc_attr( $name ); ?>" id="<?php echo esc_attr( $id ); ?>_2"
						value="<?php echo esc_attr( $value_2 ); ?>" <?php echo $data; //phpcs:ignore  ?>/>
			<?php endif; ?>

				<p><?php echo esc_html( $arg['desc'] ); ?></p>
		...
		public function attribute_save( $term_id, $tt_id, $taxonomy ) {
			if ( isset( $_POST['term_value'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
				...
				if ( is_array( $_POST['term_value'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
					...
					$value = wc_clean( wp_unslash( $_POST['term_value'] ) ); //phpcs:ignore WordPress.Security.NonceVerification, WordPress.Security.ValidatedSanitizedInput
				}

				ywccl_update_term_meta( $term_id, '_yith_wccl_value', $value );
			}
			if ( isset( $_POST['term_tooltip'] ) ) { //phpcs:ignore WordPress.Security.NonceVerification
				ywccl_update_term_meta( $term_id, '_yith_wccl_tooltip', sanitize_text_field( wp_unslash( $_POST['term_tooltip'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification
			}
		}
